...
>     TO BE ADAPTED: Data comes from a maize crop experiment (see description in Wallach et al., 2011).
...
The execution of this Rmarkdown document requires the path to the ApsimX model. By default it uses 
```{r eval=FALSE}

apsimx_path

```
(see apsimx_path param at the beginning of the .Rmd).
...
optim_options=list() 
optim_options$nb_rep <- 1 # Number of repetitions of the minimization 
...
optim_options$path_results <- param$path_results
...
Complementary graphs and data are stored in the **`r optim_options$path_results`** folder. Among them, the `EstimatedVSinit.pdf` file containing the following figures: 
...
#knitr::include_graphics(optim_options$path_results, "ResultsSimpleCase","sim_obs_plots.png")
